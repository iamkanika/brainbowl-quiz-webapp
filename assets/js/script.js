...
const answers = Array.from(
  document.getElementsByClassName("answers-text-jsRef")
);
...
const continuePlayingButton = document.querySelector(
  "#btn-continue-playing-jsRef"
);
const endGameHighScoresList = document.querySelector(
  ".endGameHighScoresList-jsRef"
);
...
const returnHomeScreenButton = document.querySelector(
  "#btn-return-to-home-screen-jsRef"
);
...
const userFinalScoreContainer = document.querySelector(
  "#user-final-score-container-jsRef"
);
const viewHighScoresButton = document.querySelector(
  "#btn-view-high-scores-jsRef"
);
...
let questions = [];
let score = 0;
let quizUrl = `https://opentdb.com/api.php?amount=${qtyOfQuestionsToFetch}&category=18&difficulty=${level}&type=multiple`;
...
window.onload = function () {
  if (sessionStorage.getItem("hasSampleScoresBeenAddedBefore") == null) {
    ...
    let letsAddSomeSampleHighScores = [
      {
        score:
          (Math.floor(Math.random() * SetQtyOfQuestions) + 1) *
          pointsPerCorrectAnswerHard,
        name: "Kanika",
      },
      {
        score:
    ...
    sessionStorage.setItem(
      "highScores",
      JSON.stringify(letsAddSomeSampleHighScores)
    );
...
function hideSubmitButtonIfLowestScore() {
  ...
  highScoresNumbers = JSON.parse(sessionStorage.getItem("highScores")).map(
    function (i) {
      return i.score;
    }
  );
...
function updateQuizLevel() {
  ...
  quizUrl = `https://opentdb.com/api.php?amount=${qtyOfQuestionsToFetch}&category=18&difficulty=${level}&type=multiple`;
  pointsPerQuestion();
...
getNewQuestion = () => {
  ...
  progressBarFull.style.width = `${
    (questionCounter / SetQtyOfQuestions) * 100
  }%`;
  if (availableQuestions.length === 0 || questionCounter >= SetQtyOfQuestions) {
  ...
  const questionIndex = Math.floor(
    Math.random() * (qtyOfQuestionsToFetch - (questionCounter - 1))
  );
